...
class TicketProcessor:
    ...
    def process_and_save(self):
        possible_columns = ["ticket_text", "text", "description", "message", "body"]
        text_col = next((col for col in self.df.columns if col.lower() in possible_columns), None)
    
        if not text_col:
            raise KeyError(f"No valid text column found! Available columns: {list(self.df.columns)}")
